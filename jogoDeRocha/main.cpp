...
void moveMouse(int x, int y) {
    if(comoJogar) {
        ...
        renderSceneJogando(fundo, personagem, hp4, hp3, hp2, hp1, hp0, vitoria, proxFase, fundoPerdeu, menuButton, mouseCimaVenceu, 1);
    } else if(jogando2) {
        if(x >= 550 && x <= 750 &&
            y >= 450 && y <= 500) {
            mouseCimaVenceu = true;
        ...
        renderSceneJogando(fundo, personagem, hp4, hp3, hp2, hp1, hp0, vitoria, proxFase, fundoPerdeu, menuButton, mouseCimaVenceu, 2);
    } else if(jogando3) {
        if(x >= 550 && x <= 750 &&
            y >= 450 && y <= 500) {
            mouseCimaVenceu = true;
        ...
        renderSceneJogando(fundo, personagem, hp4, hp3, hp2, hp1, hp0, vitoria, proxFase, fundoPerdeu, menuButton, mouseCimaVenceu, 3);
    }
}
...
void handleMouse(int button, int state, int x, int y) {
    ...
    if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN &&
        ...
        lose = false;
        menu = true;
        cout << "Botao 'MENU DPS QUE EPRDE' clicado!"<<win<< endl;
    } else if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN &&
        x >= 550 && x <= 750 &&
...
int main(int argc, char** argv) {
    ...
    vitoria            = carregaTextura("fakeVitoria.png");
    proxFase           = carregaTextura("proxima.png");
    menuButton               = carregaTextura("menu.png");
    fundoPerdeu        = carregaTextura("fakeVitoria.png");
